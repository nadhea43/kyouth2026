...
def parse_reply(reply: str):
    ...
    for line in reply.splitlines():  # split reply into lines
        ...
        clean = line.replace("JOB", "").replace(
            "Job ", ""
        )  # buang "JOB" atau "Job " kalau ada, supaya tinggal ID dan tech stack
...
def tag_data(db_url: str):
    ...
    for batch_num, batch in enumerate(chunked(rows, BATCH_SIZE)):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if not results:
                print(
                    f"[Batch {batch_num}] Attempt {attempt} failed to parse any valid job tags."
                )
                if attempt < MAX_RETRIES:
        ...
        if not success:
            print(
                f"[Batch {batch_num}] Failed after {MAX_RETRIES} attempts. Moving to next batch."
            )
            continue
